...
- (void)singleTapGestureCaptured:(UITapGestureRecognizer *)gesture
{
    ...
    if (num==1) {
        ...
        [rotateView setFrame:CGRectMake(rotateView.frame.origin.x, rotateView.frame.origin.y, sqrtf(sliderRadius.value*sliderRadius.value*scrollView.zoomScale*scrollView.zoomScale/2), sqrtf(sliderRadius.value*sliderRadius.value*scrollView.zoomScale*scrollView.zoomScale/2))];
    ...
    if (num==2) {
        [subView setFrame:CGRectMake(subView.frame.origin.x, subView.frame.origin.y,sliderWidth.value*scrollView.zoomScale, sliderWidth.value*scrollView.zoomScale)];
    ...
    if (num==3) {
        [subView setFrame:CGRectMake(subView.frame.origin.x, subView.frame.origin.y, sliderWidth.value*scrollView.zoomScale*1.5, sliderWidth.value*scrollView.zoomScale*1.5)];
        subView.center = touchPoint;
...
//    }
    ...
    if (scrollView.contentSize.height<self.view.frame.size.height) {
        ...
        if (touchPoint.x<=self.subView.frame.size.width/2 && touchPoint.y<=(self.subView.frame.size.height/2+(self.scrollView.frame.size.height-self.scrollView.contentSize.height)/2) && touchPoint.y>=(self.scrollView.frame.size.height-self.scrollView.contentSize.height)/2){
            ...
            if (num==2) {
                [rotateView setFrame:CGRectMake(0,0, sliderWidth.value*scrollView.zoomScale , sliderHeight.value*scrollView.zoomScale)];
                
                rotateView.center = touchPoint;
                
                [subView setFrame:CGRectMake(sliderWidth.value/2, sliderWidth.value/2, sliderWidth.value*scrollView.zoomScale, sliderWidth.value*scrollView.zoomScale)];
...
- (IBAction)Rectange:(id)sender {
    num = 3;
    [subView setFrame:CGRectMake(subView.frame.origin.x, subView.frame.origin.y, RectSize.size.width , RectSize.size.height)];
    subView.center = rotateView.center;
    
    [imageView setFrame:subView.frame];
    
    [rotateView setFrame:subView.frame];
